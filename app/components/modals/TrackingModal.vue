<template>
  <div
    class="group relative flex items-center justify-between gap-4 p-6 bg-white border-2 border-gray-200 rounded-2xl hover:border-green-600 hover:shadow-xl transition-all duration-300"
    :class="{ 'opacity-60 pointer-events-none': isDeleting }"
  >
    <!-- Loading Overlay -->
    <!-- <div v-if="isDeleting" class="absolute inset-0 bg-white/80 backdrop-blur-sm rounded-2xl flex items-center justify-center z-10">
      <div class="flex flex-col items-center gap-3">
        ...
        <p class="text-sm font-bold text-gray-900">Deleting...</p>
      </div>
    </div> -->
    ...
    <div class="flex flex-col md:flex-row md:items-center gap-6 flex-1">
      ...
      <div class="min-w-[200px]">
        ...
        <p class="font-mono font-bold text-black text-lg ml-10">{{ shipment.trackingId }}</p>
      </div>
      ...
      <div class="flex-1">
        ...
        <p class="font-bold text-black truncate ml-8">
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        ...
        :disabled="isDeleting"
        class="flex items-center gap-2 px-4 py-3 rounded-xl bg-black text-white font-bold hover:bg-gray-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
      >
      ...
      <button
        @click="handleDelete"
        :disabled="isDeleting"
        class="flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-red-600 text-red-600 font-bold hover:bg-red-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
      >
        <svg 
        ...
        <span class="hidden lg:inline">{{ isDeleting ? 'Deleting...' : 'Delete' }}</span>
      </button>
    </div>
  </div>
</template>
...
const handleDelete = async () => {
  ...
  // Reset after a delay (parent will remove the item)
  setTimeout(() => {
...
const getStatusClass = (status: string) => {
  const classes: Record<string, string> = {
    ...
    'Pending': 'bg-orange-100 text-orange-700 border-2 border-orange-300',
    'Cancelled': 'bg-red-100 text-red-700 border-2 border-red-300',
    'Delayed': 'bg-yellow-100 text-yellow-700 border-2 border-yellow-300',
  }
